...
//自定义方法：存在返回1，不存在返回0
function isExistAdmin(valueName) {
    var b = 1;
    $.ajax({
        url: "admin/admin/CheckExist",
        type: "post",
        dataType: "json",
        async: false,//是否异步执行
        data:{name:valueName},
...
//自定义方法：存在返回1，不存在返回0
function isExistPhone(valueName) {
  var b = 1;
  $.ajax({
      url: "admin/projectGroup/CheckPhoneExist",
      type: "post",
      dataType: "json",
      async: false,//是否异步执行
      data:{phone:valueName},
      success: function (data) {
          b = data.data;
      },
      error: function (errorMSG) {
