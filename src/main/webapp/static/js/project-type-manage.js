...
$(function(){
	...
	datagrid = $("#art-tb").datagrid({
		...
		onAfterEdit:function(index,row){
			$.ajax({
				url:'admin/projectType/AddOrUpdateProjectType',
				type:'post',
				dataType: 'json',
				data: {  
					"ptId" : row.ptId,
	...
	function doDelete(datagrid) {
		...
		//提醒用户是否是真的删除数据
		$.messager.confirm("确认消息", "您确定要删除类别【"+selectRows[0].ptName+"】吗？", function (r) {
			if (r) {
				MaskUtil.mask();
				$.ajax({
					url: "admin/projectType/DeleteProjectTypeById",
					type: "post",
					dataType: "json",
					data:{"id": selectRows[0].ptId},
					success: function (data) {
						MaskUtil.unmask();
						if(data.code == 200){
							datagrid.datagrid("reload");
