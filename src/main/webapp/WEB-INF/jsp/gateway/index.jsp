...
<body>
	...
	<div class="box">
		<div class="news left">
			<h2>
				<span class="more"><a href="article/list/${xwzxId }-1">更多..</a></span>新闻中心
			</h2>
			<ul>
				<li>
					<div class="pic_news left">
						<div class="ck-slide">
							<ul class="ck-slide-wrapper">
								<c:set value="0" var="picSum" />
								<c:forEach items="${xwzxList }" var="article">
									<c:if test="${article.artPic !='' && article.artPic !=null }">
										<c:set value="${picSum + 1}" var="picSum" />
										<li><a href="javascript:"><img
												style="width: 300px; height: 235px;"
												src="${article.artPic }" alt=""></a></li>
									</c:if>
								</c:forEach>
							...
							<div class="ck-slidebox">
								<div class="slideWrap">
									<ul class="dot-wrap">
										...
										<c:forEach var="i" begin="1" end="${picSum }" step="1">
										<c:if test="${i == 1 }">
											<li class="current"><em>1</em></li>
										</c:if>
										<c:if test="${i != 1 }">
											<li><em><c:out value="${i}" /> </em></li>
										</c:if>
										</c:forEach>
									</ul>
								</div>
						...
						<c:if test="${fn:length(xwzxList) > 0}">
							<section class="c_n_top">
								...
								<p>
									...
									[<a href="article/detail/${xwzxList[0].artId }" >详情</a>]
								</p>
							</section>
						</c:if>
						<ul>
							<c:forEach items="${xwzxList }" var="article" begin="1">
								<li><a href="article/detail/${article.artId }" >${article.artTitle }</a></li>
							</c:forEach>
						</ul>
					</div>
				</li>
